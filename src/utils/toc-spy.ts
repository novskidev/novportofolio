...
type TocWindow = Window & {
  __novskidevTocSpyCleanup?: () => void;
  __novskidevTocSpyEvents?: boolean;
  __novskidevTocSpyPageKey?: string;
};
...
const initNovskidevToc = (): (() => void) | null => {
...
export const mountNovskidevTocSpy = (): void => {
  const tocWindow = window as TocWindow;
  const cleanupCurrent = () => {
    tocWindow.__novskidevTocSpyCleanup?.();
    tocWindow.__novskidevTocSpyCleanup = undefined;
    tocWindow.__novskidevTocSpyPageKey = undefined;
  };

  const mountCurrentPage = () => {
    const pageKey = `${window.location.pathname}${window.location.search}`;
    if (tocWindow.__novskidevTocSpyPageKey === pageKey) {
      return;
    }

    cleanupCurrent();

    const cleanup = initNovskidevToc();
    if (cleanup) {
      tocWindow.__novskidevTocSpyCleanup = cleanup;
      tocWindow.__novskidevTocSpyPageKey = pageKey;
    }
  };

  if (!tocWindow.__novskidevTocSpyEvents) {
    tocWindow.__novskidevTocSpyEvents = true;
